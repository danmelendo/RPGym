...
- **Dónde va cada cosa de la parte social** (reorganizado en la 1.0.2, no lo muevas sin motivo):
  - **Inicio** = lo que caduca. `EntrenandoAhora`, `MeHanSuperado`, `NovedadesCard`, `QuedadasPanel` y `ClasificacionCard`.
  - **Pestaña “RPGym”** (es `tab==="progreso"`, `ProgressView`; el `id` interno sigue siendo `progreso` a propósito) = tu progreso + `AmigosPanel` y `CopiaNubePanel`.
  - **`AccountView`** (`tab==="cuenta"`) = solo entrar/registrarse, el nombre de usuario y cerrar sesión. Nada más: todo lo demás se le quitó.
...
- **Push (FCM) — desactivado a propósito**: `@capacitor/push-notifications` está **desinstalado**. En nativo llama a `FirebaseMessaging.getInstance()`, que **revienta la app al arrancar** si no existe `android/app/google-services.json`; eso fue el crash de la 1.0.1. Que el bridge exista NO basta como comprobación. [vite.config.js](vite.config.js) inyecta `__PUSH_CONFIGURADO__`, cierto solo si ese fichero está, y [src/push.js](src/push.js) lo exige. Para reactivarlo: seguir [supabase/FIREBASE.md](supabase/FIREBASE.md), dejar el `google-services.json` en su sitio y `npm i @capacitor/push-notifications@^7` + `npx cap sync android`.
