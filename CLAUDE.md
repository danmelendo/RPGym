...
- **Un solo archivo** `src/App.jsx`. Raíz `export default function App()`. Vistas: `HomeView`, `RoutinesView`, `RoutineBuilderView`, `WorkoutView`, `ResultsView`, `ProgressView`, `AchievementsView`, `CharacterView`, `InformeSemanalView`, `AmigosView`, `DietView`, `CustomDietEditor`, `SettingsView`, `AccountView`. Aux: `Ring`, `Toast`, `RestTimer`, `Empty`, `ExImage`, `RoutineCard`, `ExercisePicker`.
- **Dónde va cada cosa** (reorganizado en la 1.0.5, no lo muevas sin motivo):
  - **Inicio** = lo que caduca o se mira de pasada, y **poco**: aviso de vacaciones, `EntrenandoAhora`, `MeHanSuperado`, `NovedadesCard`, racha, misiones, objetivo semanal y la ficha de día de descanso. Estaba sobrecargado: las quedadas, la clasificación y los amigos se mudaron a `AmigosView`.
  - **`AmigosView`** (`tab==="amigos"`) **no tiene pestaña de la barra**: se entra por el botón de la cabecera de Inicio — el que antes abría la cuenta — y se sale con su botón de volver. Lleva `AmigosPanel`, `QuedadasPanel` en **modo "todo"** (ver, contestar y proponer en el mismo sitio) y `ClasificacionCard`. Sin cuenta enseña una tarjeta que lleva a `Ajustes → Cuenta`.
  - **Pestaña “RPGym”** (es `tab==="progreso"`, `ProgressView`; el `id` interno sigue siendo `progreso` a propósito) = **solo tu progreso**: calendario, ciclo, constancia, gráficas, marcas y mediciones. Nada social.
  - **Ajustes por pestañas** (`SETTINGS_TABS`, fila que se desliza en horizontal): `Perfil` (apariencia, datos, experiencia, ciclo) · `Entreno` (días, vacaciones, cuota del gym) · `Avisos` (recordatorios) · `Cuenta` · `Copias` · `Privacidad`. De un tirón eran diez secciones seguidas y no se encontraba nada.
  - **Las dos copias de seguridad van juntas** en la pestaña `Copias`: la de texto y `CopiaNubePanel` (cifrada). Es la misma tarea.
  - **`AccountView`** ya **no es una pestaña**: vive dentro de `Ajustes → Cuenta` con `embebido:true` (que le quita su cabecera, para no encadenar dos flechas de volver). Sigue usándose a pantalla completa con `arranque:true` como puerta de entrada. Solo entrar/registrarse, el nombre de usuario y cerrar sesión.
  - **`InformeSemanalView`** (`tab==="informe"`) **tampoco tiene pestaña**. **Sale solo** la primera vez que abres la app con la semana ya cerrada (en la práctica, el lunes) y enseña la semana **pasada**; después se consulta cuando quieras desde la **ficha de personaje**, que abre la semana **en curso**. Se sale con su botón de volver, que regresa a donde estabas (`informeVuelve`).
...
- **Dieta / bienestar**: la dieta son **ideas de comidas SIN calorías ni macros**. Se quitó la *regla del plato* (y la tarjeta del cheat day, que ahora es un día de descanso y no pinta nada aquí): está pendiente de decidir cómo seguir con las dietas, así que no vuelvas a meter reglas nutricionales sin hablarlo. No des cifras de déficit ni conteos. Mantén el descargo de dietista. Nada que fomente conductas alimentarias no saludables.
...
- **Gamificación**:
  ...
  - **Informe semanal · se RECALCULA, no se guarda.** `informeSemanal({ log, state, measures, lunesISO, plannedDays })` monta el repaso de cualquier semana a partir del historial, así que restaurar una copia o renombrar un ejercicio se refleja solo y no quedan informes viejos mintiendo. Por eso se puede pasear semana a semana hacia atrás.
    - **Los récords se reconstruyen replicando el historial en orden** (`recordsDeSemana`), comparando con la mejor marca ANTERIOR igual que hace `finishWorkout`: la primera vez que haces un ejercicio no es récord, es la marca de partida. Guardar la lista en el registro habría dejado sin informe todas las semanas ya pasadas.
    - **El cardio es la excepción**: el registro no guarda si el objetivo iba en minutos o en segundos, así que su historial de marcas no se puede reconstruir. Se usa la fecha del récord vigente en `state.cardioBests`. Limitación conocida y a propósito.
    - **`setsPorGrupoEnRango`, no `weeklySetsByGroup`**: el segundo no tiene tope superior y se llevaría por delante todo lo entrenado después de la semana que se está mirando.
    - **Tus números salen del móvil; la actividad de los amigos va aparte** (`cloud.novedades(desde, { hasta, limite })`). Sin cuenta, sin red o con la nube caída el informe sale entero, solo sin la sección "Tu círculo". No metas datos de red en el cálculo local.
    - La rejilla de días usa **los mismos colores que `StreakCard`** (dorado previsto, jade recuperado, luna descanso, llama hoy). Dos leyendas distintas para lo mismo no las aprende nadie.
    - **`gym:informeVisto`** (global) guarda el lunes de la última semana ya enseñada: por eso no vuelve a salir por mucho que abras y cierres. No se enseña si esa semana no hay nada que contar, ni antes de pasar la pantalla de entrada (si no, se daría por visto detrás del login).
    - Probado con fechas fijas en `scratchpad/probar_informe.mjs`, que extrae las funciones reales de `src/App.jsx` (no una copia) y las ejecuta con un "hoy" congelado.
  - **Días de descanso y vacaciones: dos formas de bajar el listón sin perder la racha.** Las dos entran por `semanaOpts(state)` → `resumenSemana(log, dias, lunes, opts)`, y de ahí las leen la racha, las misiones, el objetivo y el informe. **Si añades otro sitio que cuente semanas, pásale `semanaOpts` o contará distinto que el resto.**
    - **Día de descanso** (`restTokens`, antes "cheat day"): se gana cumpliendo la semana y se canjea para que ESA semana pida una sesión menos (`restWeeks[lunes] = 1`). **Uno por semana como mucho**: encadenarlos borraría la semana entera y la racha dejaría de significar nada. Tampoco se deja gastar en una semana ya cumplida — sería tirar la ficha. Si al canjearlo la semana queda cumplida, se da por cumplida y la racha sigue, pero **sin XP ni ficha nueva**: eso se gana entrenando, y regalarlo sería un bucle de fichas infinitas.
    - Ya no es una galleta ni tiene nada que ver con la dieta: el icono es `BedDouble` y la tarjeta vive en Inicio. Se migra desde `cheatTokens` al cargar el perfil, así que una copia de seguridad vieja no pierde nada.
    - **Vacaciones** (`state.vacations`, lista de rangos `{from,to}`): esos días no se prevé ninguna sesión, así que una semana entera de vacaciones sale con `previstos:0`, cuenta como cumplida y **no corta la racha** (`habitStreak` no rompe en una semana vacía si fue de vacaciones). Los recordatorios se callan mientras duran.
    - Se guarda la **lista** de rangos, no solo el último, para que la racha siga saliendo bien al recalcularla meses después. "Volver ya" recorta el rango en curso a ayer en vez de borrarlo.
