...
- **`AmigoView`** (`tab==="amigo"`, se abre tocando un amigo en `AmigosPanel`): su nivel y rango, qué entrena, sus marcas comparadas con las tuyas, y los dos botones de acción (quedar con él, pasarle una rutina). Sale con su botón de volver, que devuelve a la pestaña RPGym.
  - **Los atributos de un amigo NO los calcula el servidor**: se derivan en el móvil cruzando sus `exercise_records` con `EX_MUSCLE` y `BODY_MAP`. El servidor guarda "ejercicio + peso + veces" y no sabe de músculos. Si un ejercicio suyo no está en tu `EX_MUSCLE` (versión más nueva de la app), no cae en ningún grupo pero sigue apareciendo en "sus marcas": es la degradación buscada, no un fallo.
